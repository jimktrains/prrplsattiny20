...
// pcm1723
// Keri DuPrey
// Nat Blundelltpi_identify_device
//
...
#define SOFT_UART_RX A3
#define SOFT_UART_TX A2
#define SOFT_UART_BAUD 300

SoftwareSerial soft_uart = SoftwareSerial(SOFT_UART_RX, SOFT_UART_TX);

void setup() {
  tristate_arduino_spi();  // so we can test the tiny at power on

  Serial.begin(115200);
  soft_uart.begin(SOFT_UART_BAUD);
  disableSoftUart();
}

void disableSoftUart() {
  soft_uart.stopListening();
  pinMode(SOFT_UART_RX, INPUT);
  pinMode(SOFT_UART_TX, INPUT);
}

void enableSoftUart() {
  pinMode(SOFT_UART_RX, INPUT);
  pinMode(SOFT_UART_TX, OUTPUT);
  digitalWrite(SOFT_UART_TX, HIGH);
  soft_uart.listen();
}

void soft_uart_console() {
  int16_t c;
  enableSoftUart();
...
uint8_t cycle_signals = 0;
uint32_t last_cycle_millis = 0;
auto last_keypress = 0;
void loop() {
  Keypad kpad(5, 9);
  memset(msg, 0, 5);
  if (show_prompt) {
    Serial.print(F("Command [d,e,i,m,u,v,s,?] > "));
  }
  show_prompt = true;

  while (Serial.available() < 1) {
    auto k = kpad.scan();
    if (k && k != last_keypress) {
      memset(msg, 0, 5);
      cycle_signals = 0;
      last_cycle_millis = -1;

      Serial.print("Keypad press: ");
      Serial.print(k);
      Serial.print(" ");
      Serial.print(k == Keypad::KEY_1);
      Serial.print(" ");
      Serial.print(k == Keypad::KEY_2);
      Serial.print(" ");
      Serial.print(k == Keypad::KEY_3);
      Serial.print(" ");
      ...
      switch (k) {
        case Keypad::KEY_1:
          msg[1] = 'S';
          Serial.println("0 STOP from keypad");
          break;
        case Keypad::KEY_2:
          msg[1] = 'A';
          Serial.println("0 APPR from keypad");
          break;
        case Keypad::KEY_3:
          msg[1] = 'C';
          Serial.println("0 CLEAR from keypad");
          break;
        case Keypad::KEY_4:
          msg[2] = 'S';
          Serial.println("1 STOP from keypad");
          break;
        case Keypad::KEY_5:
          msg[2] = 'A';
          Serial.println("1 APPR from keypad");
          break;
        case Keypad::KEY_6:
          msg[2] = 'C';
          Serial.println("1 CLEAR from keypad");
          break;
        case Keypad::KEY_7:
          msg[3] = 'S';
          Serial.println("2 STOP from keypad");
          break;
        case Keypad::KEY_8:
          msg[3] = 'A';
          Serial.println("2 APPR from keypad");
          break;
        case Keypad::KEY_9:
          msg[3] = 'C';
          Serial.println("2 CLEAR from keypad");
          break;
        case Keypad::KEY_0:
          cycle_signals = 1;
          last_cycle_millis = millis() - 3000;
          Serial.println("cycle from keypad");
          break;
    ...
    last_keypress = k;
    if (cycle_signals && ((last_cycle_millis - millis()) > 2000)) {
      Serial.print("cycle_signals=");
      Serial.println(cycle_signals);
      msg[1] = ((cycle_signals + 1) % 3) + 1;
      msg[2] = ((cycle_signals + 2) % 3) + 1;
      msg[3] = ((cycle_signals + 3) % 3) + 1;
      cycle_signals = ((cycle_signals + 1) % 3) + 1;
    }
  ...
  switch (r) {
    case 'd':
    case 'D': tpi_dump_memory(); break;
    case 'e':
    case 'E': tpi_chip_erase(); break;
    case 'i':
    case 'I': tpi_identify_device(); break;
    case 'm':
    case 'M': print_free_memory(); break;
    case 'u':
    case 'U': upload_program(); break;
    case 'v':
    case 'V': print_version(); break;
    case 's':
    case 'S': soft_uart_console(); break;
    case ' ':
    case '\t':
    case '\r':
    case '\n': show_prompt = false; break;
    ...
    default:
      {
        ...
        Serial.println(F("S: Serial Console(RX=A3 TX= BAUD=300 8N1)"));
...
void upload_program() {
  ...
  do {
    ...
    serial_decode_byte();
    bytesconsumed++;
    bytesconsumed++;  // consume checksum
  } while (type != kIHTypeEOF);

  Serial.println(F("Upload complete"));
  Serial.println(bytesconsumed);
...
void tpi_dump_memory() {
  ...
  while (gTPIPointer < flashEnd) {
    ...
    if (
      gTPIPointer >= kTiny4IO && gTPIPointer < kTiny4Reserved0 || gTPIPointer >= kTiny4NVMLock && gTPIPointer < kTiny4Reserved1 || gTPIPointer >= kTiny4Configuration && gTPIPointer < kTiny4Reserved2 || gTPIPointer >= kTiny4Calibration && gTPIPointer < kTiny4Reserved3 || gTPIPointer >= kTiny4DeviceId && gTPIPointer < kTiny4Reserved4 || gTPIPointer >= kTiny4Flash && gTPIPointer < flashEnd) {
      if (
        gTPIPointer == kTiny4IO || gTPIPointer == kTiny4NVMLock || gTPIPointer == kTiny4Configuration || gTPIPointer == kTiny4Calibration || gTPIPointer == kTiny4DeviceId || gTPIPointer == kTiny4Flash) {
...
bool tpi_identify_device() {
  ...
  if (gATtinyType) {
    ...
    Serial.print(F("Unable to identify device "));
    Serial.print(b0, HEX);
    Serial.print(" ");
