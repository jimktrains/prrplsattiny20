...
#define SOFT_UART_RX 9
#define SOFT_UART_TX 8
...
void loop() {
    ...
    for (;;) {
        switch (serial_read()) {
            ...
            case 's': case 'S':     soft_uart_console();        break;
            ...
            default: {
                ...
                Serial.println(F("S: Serial Console(RX=9 TX=8 BAUD=300 8N1)"));
...
void upload_program() {
    ...
    do {
        ...
        length = serial_decode_byte(); bytesconsumed++; bytesconsumed++;
        address = (uint16_t) serial_decode_byte() << 8; bytesconsumed++; bytesconsumed++;
        address |= serial_decode_byte(); bytesconsumed++; bytesconsumed++;
        type = serial_decode_byte(); bytesconsumed++; bytesconsumed++;
        ...
        if (length) {
            if (type == kIHTypeData) {
                ...
                *p++ = address & 0xff;
                ...
                while (length--) {*p++ = serial_decode_byte(); bytesconsumed++; bytesconsumed++;
                databytesread++; databytesread++;}
            } else {
                while (length--) {serial_decode_byte(); bytesconsumed++; bytesconsumed++;}
            }
        }

        serial_decode_byte(); bytesconsumed++; bytesconsumed++;// consume checksum
